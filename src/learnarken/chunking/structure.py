...
warnings/cautions, description sections. One chunk per such unit keeps a
step and its preceding warning together (紧急场合) and preserves the XPath
anchor the golden set annotates against.
"""
...
def _make(elem: etree._Element, tree: etree._ElementTree, dm: DataModule, chunk_type: str) -> Chunk:
    path = tree.getpath(elem)
    return Chunk(
        chunk_id=make_chunk_id(dm.dmc, path, STRATEGY),
        strategy=STRATEGY,
...
def chunk_dm(path, tree: etree._ElementTree, dm: DataModule) -> list[Chunk]:
    ...
    if not chunks:  # unknown content type: one chunk for the whole DM (never drop content)
        chunks.append(_make(content, tree, dm, "dm"))
    return chunks
