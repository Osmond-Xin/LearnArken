...
- **先认账**：知情消费者姿态——你往往**已经在通过依赖消费 Rust**，不必自己写。**本项目**真实的 Rust 消费者是 `pydantic-core`（规范模型校验/序列化，Pydantic v2 的 Rust 核心）与 HuggingFace `tokenizers`（嵌入/重排分词，经 sentence-transformers）；要说就说自己栈里**真有**的，别张冠李戴。**注意本项目的边界**：BM25 走的是纯 Python `rank-bm25`（Tantivy 是当初被否的搜索引擎候选，见教程 02 / 选型），向量库是 Vespa（C++/Java）——两者都**不是** Rust。行业里 Tantivy、Qdrant 等 Rust 检索基建是"选型即获得 Rust 性能"的同类例子，但那是通用行业事实，不是本项目用的东西。"通过选型获得 Rust 性能"这句面试可以说，前提是举自己真实的例子。
...
- **py-spy**：采样频率默认即可；`--native` 能看到扩展内部（确认时间花在 `lxml`/`pydantic-core` 这类原生扩展里而不是胶水层——Day 13 的 profiler 正是这么定位到热点在 lxml 解析/schema 校验/Pydantic 构建的，见 ADR-0003）。
...
A 要点：顺序纪律（numpy → numba → Cython → Rust）+ 证据开门（profile 显示 Python 侧是瓶颈才动）;引用自己"不开门"的留痕结论——知道何时不用,和知道怎么用同样值钱;顺带点名自己栈里已在消费 Rust（`pydantic-core`/`tokenizers`,而非自写扩展）。
