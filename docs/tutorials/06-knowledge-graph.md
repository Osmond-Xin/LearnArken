...
**选型上下文**（为什么本项目押 RDF，§6 有细节）：领域是标准驱动 + 多源整合（S1000D/iSpec/S2000M 要合并——IRI 让合并 = 三元组并集）、要版本化（命名图白送）、要推理（传递闭包白送）。**如果没有这三个需求，Neo4j 甚至 Postgres 递归 CTE 都是更省事的选择**——技术没有高低，匹配才有。
