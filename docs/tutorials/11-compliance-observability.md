...
- **查询时过滤**：用户资质进检索过滤器——**权限过滤必须在检索层，不能靠 prompt 叮嘱 LLM"别泄密"**（教程 05 讲过：进了上下文就可能进答案）。这句是面试金句级的架构立场。本项目已落地的检索层过滤是 **Vespa 引擎侧 scope/attribute 过滤 + fail-closed 越界校验**（按 package/适用性）；行业里还有 Qdrant payload filter、SPARQL FILTER 等形态。（基于用户密级资质的 RBAC 过滤是 Roadmap。）
...
数据分级 + 检索层过滤          金句级架构立场的载体，Vespa scope/attribute 过滤白送机制
