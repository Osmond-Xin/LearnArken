...
flowchart TD
    ...
    B -- 是 --> H["HNSW<br/>（Qdrant 默认，本项目主路径）"]
...
- **FAISS 是库**：一堆索引算法的 C++ 实现 + Python 绑定。没有服务、没有过滤、没有持久化管理。适合做实验（LearnArken 的 ANN lab 用它跑 Flat/HNSW/IVF/IVFPQ 对比就是标准玩法）。
- **Qdrant 是数据库**：HNSW 索引 + **payload 元数据过滤** + API 服务 + 持久化 + 分片。生产主路径用它。
- **过滤 + ANN 的组合是真难题**（面试亮点）：`WHERE model='X' AND vector近邻` —— 先过滤再暴力搜（过滤结果少时好）？还是边走图边跳过不合条件的点（过滤结果多时好）？Qdrant 会根据过滤基数自动选策略，并额外为 payload 建索引。理解这个问题的存在本身就是加分项，这直接对应 LearnArken 的"适用性过滤"需求。
...
**Q: Qdrant 为什么比直接用 FAISS 适合做产品？**
A 要点：payload 过滤（适用性/版本/密级过滤是业务刚需，且过滤+ANN 是 Qdrant 优化过的难题）、服务化 API、持久化与快照、分片复制。FAISS 定位是算法库，适合离线实验。两者不是竞争关系。
