# 05 · 服务化与 Demo：FastAPI + Streamlit（Day 6）

> **AI-drafted，待人审**。快照：2026-07-17（Day 6 实现完成、红队两轮已裁、
> 待提交）。本篇讲 CLI→HTTP 服务化的架构：一个 FastAPI 后端 + 一个
...
> [research/day6-unknowns](../research/day6-unknowns.md)。
...
| 面 | 措施 | 出处 |
| --- | --- | --- |
| 网络暴露 | 仅 bind `127.0.0.1`(两服务);不上公网,故不做限流/JWT/预算熔断(超范围) | 扫描 §未知的已知 |
...
| 层 | 说明 |
| --- | --- |
| 工程化(可信) | 上传磁盘事务化;SSE 召回覆盖门拒答+传输中断;CSRF Origin 门;fail-closed 预检;哑客户端结构性保证 |
| 玩具层(已标注) | 单用户单 worker;`_corpus_lock` 粗粒度(查询racing换空窗最坏 fail-closed);无 request_id↔trace_id 贯穿;无鉴权(loopback 前提) |
| 明确超范围 | 公网部署面(限流/JWT/熔断);多轮记忆;`GET /traces/{id}`;async 全栈重构;index-epoch(#8) |
