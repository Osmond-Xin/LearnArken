...
class TestTheBackendRecordsWhatWasAsked:
    def test_an_answered_query_is_recorded(self, client, public, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.INFO, logger="learnarken"):
    ...
    def test_the_turn_id_pairs_the_two_lines(self, client, public, monkeypatch, caplog):
        ...
        assert _access_lines(caplog)[0]["turn"] == "abc-123"
    ...
    def test_a_hostile_turn_header_is_stripped(self, client, public, monkeypatch, caplog):
        ...
        turn = _access_lines(caplog)[0]["turn"]
        assert len(turn) <= 16
        assert re.fullmatch(r"[A-Za-z0-9_-]*", turn)

    def test_info_actually_reaches_stderr_in_public_mode(self, public, capsys, monkeypatch):
        """Not `caplog`. Python's root level is WARNING and uvicorn configures
        only its own loggers, so on the deployed process `logger.info` was
        measured to produce *nothing* — the feature would have passed every
        test above and recorded zero visits in production (red team P1)."""
        existing = list(api.logger.handlers)
        level = api.logger.level
        monkeypatch.setattr(api.logger, "handlers", existing.copy())
        try:
        ...
        payload = json.loads(capsys.readouterr().err.split(" INFO ", 1)[1])
        assert payload["event"] == "demo_query"
    ...
    def test_enabling_the_access_log_twice_adds_one_handler(self, public, monkeypatch):
        ...
        monkeypatch.setattr(api.logger, "handlers", list(api.logger.handlers))
        before = len(api.logger.handlers)
        api.enable_public_access_log()
        api.enable_public_access_log()
        assert len(api.logger.handlers) == before + 1
