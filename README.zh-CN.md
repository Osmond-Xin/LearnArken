...
| **测试** | 585 条——`make test`(即 pytest)离线跑 `576 passed, 12 skipped`(CI 即此环境);本地 Vespa + Neo4j 起着时 `579 passed, 9 skipped`。两个数字都是 2026-07-28 实跑测得,不是算出来的。lint 是单独的 `make lint` |
...
make lint && make test                         # ruff,再 pytest → 576 passed, 12 skipped(离线)
