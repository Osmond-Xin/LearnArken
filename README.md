...
| **Test suite** | 569 tests — `make test` (pytest) → `557 passed, 12 skipped` offline, which is what CI runs; `560 passed, 9 skipped` with the local Vespa + Neo4j services up. Both measured 2026-07-28, both by running them. Lint is the separate `make lint` |
...
make lint && make test                         # ruff, then pytest → 557 passed, 12 skipped (offline)
