...
| **Test suite** | 498 tests — `make test` (pytest) → `486 passed, 12 skipped` offline, which is what CI runs; `489 passed, 9 skipped` with the local Vespa + Neo4j services up. Both measured 2026-07-27, both by running them. Lint is the separate `make lint` |
...
make lint && make test                         # ruff, then pytest → 486 passed, 12 skipped (offline)
