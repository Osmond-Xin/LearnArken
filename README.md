...
| **Test suite** | 575 tests — `make test` (pytest) → `569 passed, 12 skipped` offline, which is what CI runs; `572 passed, 9 skipped` with the local Vespa + Neo4j services up. Both measured 2026-07-28, both by running them. Lint is the separate `make lint` |
...
make lint && make test                         # ruff, then pytest → 569 passed, 12 skipped (offline)
